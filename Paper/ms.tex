...
probability distributions indexed by two hyperparameters. These
problems arise frequently in statistical mechanics.}
...
tuning parameters, and still works on problems involvin phase transitions.
...
Diffusive Nested Sampling \citep{dnest}) has these properties but for a
single objective function.

\section{Rationale}\label{sec:rationale}

{\bf Here is where I'll attempt to justify the algorithm}.

Consider the following proposal. Imagine running standard NS with $N$ particles
...
attempting to increase $L_2$ instead of $L_1$. After $M_2$ such iterations,
...
Try to relate this to \citet{vertical}.
...
\begin{eqnarray}
Y(\xx) &=& -\ln\left[X\left(L_1(\xx), L_2(\xx)\right)\right].
\end{eqnarray}
