...
\abstract{We present a generalization of the Nested Sampling algorithm,
which we call ``{\em TwinPeaks}'',
that estimates
...
$\beta_2$. Problems like this
arise frequently in statistical mechanics and occasionally in
Bayesian inference.
We demonstrate the algorithm on three example problems.
The first is a toy problem whose structure allows us
to accurately calculate $Z(\beta_1, \beta_2)$ numerically using non-Monte-Carlo
methods.
We show empirically how the accuracy of TwinPeaks
improves as a function of the number of particles used, $N$. We also
...
same function as the inputs typically don't have the same dimensionality
($\xx$ is usually a collection of quantities and $X$ is a single real number
in $[0, 1]$).
