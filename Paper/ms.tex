...
value of $\ln B$. Rather, $\ln B$ has a prior distribution:
\begin{eqnarray}
P(\ln B \in R) &=& \int P(D)P(\ln B \in R | D) \, dD\\
&=& \int P(D) \mathds{1}\left(\ln B(D) \in R\right) \, dD\\
&=& \int \sum_{i\in\{1,2\}}\left[P(M_i)P(D|M_i)\right] \mathds{1}\left(\ln B(D) \in R\right) \, dD
\end{eqnarray}
